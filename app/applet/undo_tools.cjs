...
// 3. Remove handlers
const handlersSearch = `  const handleTogglePlay = () => {
...
content = content.replace(handlersSearch, "  const handleRemoveAudio = () => {");

// 4. Revert Audio element
content = content.replace(
  "onEnded={() => setIsAudioPlaying(false)}\n        className=\"hidden\"\n        muted={isPlaybackMuted}\n      />",
  "onEnded={() => setIsAudioPlaying(false)}\n        className=\"hidden\"\n      />"
);

// 5. Revert Bottom Status Pill
...
content = content.replace(newControlBar, bottomStatusPillStr);

fs.writeFileSync('src/components/UniversalMotionTools.tsx', content);
console.log("Undo update_tools.cjs complete");
