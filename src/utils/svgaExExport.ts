...
export const handleSvgaExExport = async (params: {
  ...
  try {
    ...
    if (metadata.videoItem) {
      ...
      for (const [key, val] of Object.entries(viImages)) {
        if (typeof val === "string") {
          const base64 = (val as string).startsWith("data:")
            ? (val as string).split(",")[1]
            : (val as string);
      ...
      throw new Error(
        "Unable to initialize SVGA export: No video data available.",
      );
    }
    ...
    // Track original audio keys to prevent them from being treated as images if audio is replaced/removed
    const originalAudioKeys = new Set<string>();
    if (message.audios) {
      message.audios.forEach((a: any) => {
    ...
    const sourceImages =
      message.images || (metadata.videoItem ? metadata.videoItem.images : {});
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      const needsQualityCompression =
        globalQuality === "low" || globalQuality === "medium";
      ...
      if (
        ...
        if (globalQuality === "medium" && targetScale > 0.85)
          targetScale = 0.85;
        ...
        if (ctx) {
          ...
          if (needsQualityCompression) {
            ...
            for (let j = 0; j < data.length; j += 4) {
              ...
              data[j + 1] = Math.round(
                Math.round(data[j + 1] / factor) * factor,
              );
              data[j + 2] = Math.round(
                Math.round(data[j + 2] / factor) * factor,
              );
            }
    ...
    // Ensure all sprites have an image entry (fix for missing layers causing decode errors)
    if (message.sprites) {
    ...
    for (const layer of customLayers
      ...
      .reverse())
      await processLayer(layer, true);
    for (const layer of customLayers.filter((l) => l.zIndexMode === "front"))
      await processLayer(layer, false);
    ...
    if (watermark) {
      ...
      const wmFrame = {
        alpha: 1.0,
        layout: {
          x: videoWidth / 2 + wmPos.x,
          y: videoHeight / 2 + wmPos.y,
          width: videoWidth * wmScale,
          height: videoWidth * wmScale,
        },
        transform: { a: 1, b: 0, c: 0, d: 1, tx: 0, ty: 0 },
      };
      if (!message.sprites) message.sprites = [];
