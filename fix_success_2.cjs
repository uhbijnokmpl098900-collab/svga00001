...
const successSectionRegex = /<div className="flex items-center gap-2 text-emerald-400 font-bold">[\s\S]*?<\/div>/;
...
code = code.replace(/\{!muteOriginalAudio && \(/g, exportStatsUI + '\n                  {!muteOriginalAudio && (');

fs.writeFileSync(path, code);
console.log('Fixed success UI 2');
